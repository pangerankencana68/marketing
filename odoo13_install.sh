...
#################################################################
xmlrpc_port = $PORT
dbfilter = odoochain13
logrotate = True
limit_time_real = 1000
limit_time_cpu = 1000
" | sudo tee --append $PATHBASE/config/odoo$VCODE.conf

sudo rm /etc/systemd/system/odoochain$VCODE.service
sudo touch /etc/systemd/system/odoochain$VCODE.service
sudo chmod +x /etc/systemd/system/odoochain$VCODE.service
echo "
[Unit]
...
" | sudo tee --append /etc/systemd/system/odoochain$VCODE.service
sudo systemctl daemon-reload
sudo systemctl enable odoochain$VCODE.service
sudo systemctl start odoochain$VCODE
